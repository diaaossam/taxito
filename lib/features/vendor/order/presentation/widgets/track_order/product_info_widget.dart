...
class OrderDetailsProduct extends StatefulWidget {
  ...
  const OrderDetailsProduct({super.key, required this.orders});
      ...
      child: Column(
        ...
        children: [
          ...
          CustomButton(
            ...
            text: showDetails
                ? context.localizations.hideOrderDetails
                : "${context.localizations.showOrderDetails}  (${widget.orders.items?.length})",
            press: () => setState(() {
          ...
          SizedBox(height: SizeConfig.bodyHeight * .02),
          if (showDetails)
            ...List.generate((widget.orders.items ?? []).length, (index) {
