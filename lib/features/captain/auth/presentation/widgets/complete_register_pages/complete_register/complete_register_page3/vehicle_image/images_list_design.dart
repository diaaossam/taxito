...
import '../../../../../../../../../widgets/image_picker/pick_multi_media_sheet.dart';

class VehicleImagePickerWidget extends StatefulWidget {
  final Function(List<String>) images;
  final List<String> imagesListFromApi;

  const VehicleImagePickerWidget({
    super.key,
  ...
  @override
  State<VehicleImagePickerWidget> createState() =>
      _VehicleImagePickerWidgetState();
}

class _VehicleImagePickerWidgetState extends State<VehicleImagePickerWidget> {
  List<String> images = [];
    ...
    if (widget.imagesListFromApi.isNotEmpty) {
      return GestureDetector(
        ...
        child: Column(
          children: [
            ...List.generate(
              (images.length / 5).ceil(),
              (rowIndex) {
                final start = rowIndex * 5;
                final end = (start + 5).clamp(0, images.length);
                final rowImages =
                    images.sublist(start, end); // نجيب الصور بتاعت كل صف
                return Row(
