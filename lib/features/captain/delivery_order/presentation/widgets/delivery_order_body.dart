...
class _DeliveryOrderBodyState extends State<DeliveryOrderBody>
  ...
  void initState() {
    if(widget.id != null){
      context.read<DeliveryActionsCubit>().getOrderDetails(id: widget.id!.toInt());
    }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<DeliveryActionsCubit, DeliveryActionsState>(
                ...
                child: BlocBuilder<DeliveryOrderCubit, DeliveryOrderState>(
                  builder: (context, state) {
                    ...
                    return TabBarView(controller: controller, children: [
                      ...
                      DeliveryOrderList(
                        onGetDetails: (p0) =>
                            context.navigateTo(TrackOrderScreen(id: p0.id ?? 0)),
                        pagingController:
                            orderBloc.pagingControllerInPrepareSupplier,
                      ),
                      DeliveryOrderList(
                        statusText: context.localizations.receiveFromSupplier,
                        pagingController: orderBloc.pagingControllerDonePrepare,
                        onChangeStatus: (p0) => bloc.updateOrder(
                            id: p0.id ?? 0,
                            status: "delivery_received_order_from_supplier"),
                        onGetDetails: (p0) =>
                            context.navigateTo(TrackOrderScreen(id: p0.id ?? 0)),
                      ),
                      DeliveryOrderList(
                        pagingController: orderBloc.pagingControllerWithDelegate,
                        onChangeStatus: (p0) => bloc.updateOrder(
                            id: p0.id ?? 0, status: "delivery_completed"),
                        statusText: context.localizations.deliveryToAddress,
                        onGetDetails: (p0) =>
                            context.navigateTo(TrackOrderScreen(id: p0.id ?? 0)),
                      ),
                      DeliveryOrderList(
                        pagingController: orderBloc.pagingControllerDelivered,
                      ...
                      DeliveryOrderList(
                        ...
                        onChangeStatus: (p0) {},
                        pagingController: orderBloc.pagingControllerCanceled,
  ...
  void _init({bool isRefresh = false}) {
    ...
    if (!isRefresh) {
      bloc.pagingControllerNew.addPageRequestListener(
        (pageKey) {
          bloc.fetchPage(
      ...
      bloc.pagingControllerInPrepareSupplier.addPageRequestListener(
        (pageKey) {
          bloc.fetchPage(
      ...
      bloc.pagingControllerDonePrepare.addPageRequestListener(
            (pageKey) {
          bloc.fetchPage(
              DeliveryParams(
                  status: OrderType.readyForDeleivery, pageKey: pageKey),
              bloc.pagingControllerDonePrepare);
        },
      );

      bloc.pagingControllerWithDelegate.addPageRequestListener(
            (pageKey) {
          bloc.fetchPage(
              DeliveryParams(
                  status: OrderType.outForDeleivery, pageKey: pageKey),
              bloc.pagingControllerWithDelegate);
        },
      );

      bloc.pagingControllerDelivered.addPageRequestListener(
        (pageKey) {
          bloc.fetchPage(
      ...
      bloc.pagingControllerCanceled.addPageRequestListener(
        (pageKey) {
          bloc.fetchPage(
