...
import '../../../domain/usecases/accept_delivery_order_use_case.dart';
import '../../../domain/usecases/reject_delivery_order_use_case.dart';
...
class DeliveryActionsCubit extends Cubit<DeliveryActionsState> {
  final AcceptDeliveryOrderUseCase acceptDeliveryOrderUseCase;
  final RejectDeliveryOrderUseCase rejectDeliveryOrderUseCase;
  final UpdateDeliveryOrderUseCase updateDeliveryOrderUseCase;
  final GetOrderDetailsUseCase getOrderDetailsUseCase;
  final SocketService socketService;

  DeliveryActionsCubit(
    this.acceptDeliveryOrderUseCase,
    this.updateDeliveryOrderUseCase,
    this.socketService,
    this.rejectDeliveryOrderUseCase,
  ...
  Future<void> rejectOrder({required Orders order}) async {
    ...
    if (!isClosed) {
      await rejectDeliveryOrderUseCase(id: orderId);
  ...
  Future<void> updateOrder({String? status, required num id}) async {
    ...
    if (status == null) {
      cancelOrderTimer(id);
      response = await acceptDeliveryOrderUseCase(id: id);
    } else {
