...
class ChatsRemoteDataSourceImpl implements ChatsRemoteDataSource {
  ...
  ChatsRemoteDataSourceImpl(
      {required this.dioConsumer, required this.socketService});
  ...
  Future<ApiSuccessResponse> sendChat(
    ...
    if (sendChatParams.tripId == 0) {
      final response = await dioConsumer.post(
          path: EndPoints.supportChat, body: sendChatParams.toJson());
      MessageModel model = MessageModel.fromJson(response['data']);
      return ApiSuccessResponse(data: model);
    }
    final response = await dioConsumer.post(
        path: EndPoints.sendChat, body: sendChatParams.toJson());
    socketService.emitEvent("sendMessage",
        {"chat_uuid": sendChatParams.chatUuid, "message": response['data']});
    MessageModel model = MessageModel.fromJson(response['data']);
    return ApiSuccessResponse(data: model);
  }
  ...
  Future<ApiSuccessResponse> getChatMessages(
    ...
    if (tripModel.id == null) {
      ...
      final response = await dioConsumer
          .get(path: "${EndPoints.supportChat}/${data['data']['id']}", params: {
        "page": pageKey,
        "perPage": chaPageSize,
      });
      ...
      if (tripModel.chatId != null) {
        final response = await dioConsumer.get(
            path: "${EndPoints.sendChat}/${tripModel.chatId?.id}/messages",
            params: {
              "page": pageKey,
              "perPage": chaPageSize,
            });
