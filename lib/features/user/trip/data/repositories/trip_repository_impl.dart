...
class TripRepositoryImpl implements TripRepository {
  ...
  Future<Either<Failure, ApiSuccessResponse>> requestTrip(
      {required TripParams tripParams}) async {
    try {
      final response =
          await tripRemoteDataSource.requestTrip(tripParams: tripParams);
  ...
  Future<Either<Failure, ApiSuccessResponse>> reportIssue(
      {required String msg, required TripModel tripModel}) async {
    try {
      final response = await tripRemoteDataSource.reportIssue(
          msg: msg, tripModel: tripModel);
      return right(response);
    } catch (error) {
      return left(ServerFailure(msg: error.toString()));
    }
  ...
  Future<Either<Failure, ApiSuccessResponse>> sendTripReview(
      {required ReviewTripParams reviewParams}) async {
    try {
      final response =
          await tripRemoteDataSource.sendTripReview(reviewParams: reviewParams);
      return right(response);
    } catch (error) {
      return left(ServerFailure(msg: error.toString()));
    }
  ...
  Future<Either<Failure, ApiSuccessResponse>> sendPaymentUserRequest(
      {required num id, required String paymentMethod}) async {
    try {
  ...
  Future<Either<Failure, ApiSuccessResponse>> validateCoupon(
    ...
    try {
      final response =
          await tripRemoteDataSource.validateCoupon(discountCode: discountCode);
      return right(response);
    } catch (error) {
      return left(ServerFailure(msg: error.toString()));
    }
  ...
  Future<Either<Failure, ApiSuccessResponse>> getUserDriver(
      {required LatLng lat, required double radius}) async {
    try {
      final response =
          await tripRemoteDataSource.getUserDriver(lat: lat, radius: radius);
      return right(response);
    } catch (error) {
      return left(ServerFailure(msg: error.toString()));
    }
  ...
  Future<Either<Failure, ApiSuccessResponse>> getTripByUuid({
    ...
    try {
      final response = await tripRemoteDataSource.getTripByUuid(
        uuid: uuid,
      );
      return right(response);
    } catch (e) {
  ...
  Future<Either<Failure, ApiSuccessResponse>> getAllTrips(
      ...
      required bool isSch}) async {
    try {
      final response = await tripRemoteDataSource.getAllTrips(
          status: status, pageKey: pageKey, isSch: isSch);
      return right(response);
    } catch (e) {
      return left(ServerFailure(msg: e.toString()));
    }
