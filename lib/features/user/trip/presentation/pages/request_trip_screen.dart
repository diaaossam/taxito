...
class RequestTripScreen extends StatelessWidget {
  ...
  const RequestTripScreen({super.key, this.tripModel});

  @override
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (context) => sl<TripBloc>()..getCurrentLocation()),
        BlocProvider(create: (context) => sl<AcceptedUserTripCubit>()),
        BlocProvider(create: (context) => sl<RequestTripBloc>()),
        BlocProvider(create: (context) => sl<TripRatingBloc>()),
      ],
      child: Scaffold(
        body: SafeArea(child: TripBody(tripModel: tripModel)),
      ),
    );
  }
}
