...
import 'package:aslol/features/order/domain/usecases/get_delivery_cost_use_case.dart';
...
class MyAddressCubit extends Cubit<MyAddressState> {
  ...
  MyAddressCubit(this.getAddressUseCase, this.makeAddressDefaultUseCase,
      this.deleteAddressUseCase, this.getDeliveryCostUseCase)
  ...
  Future<void> getMyAddress() async {
    ...
    emit(response.fold(
      (l) => GetMyAddressFailure(msg: l.msg),
      (r) {
        addressList = r.data;
  ...
  Future<void> makeAddressDefault({required MyAddress myAddress}) async {
    ...
    emit(response.fold(
      ...
      (r) =>
          MakeAddressDefaultSuccess(msg: r.message ?? "", myAddress: myAddress),
    ));
  }
  ...
  Future<void> deleteAddress({required num id}) async {
    ...
    emit(response.fold(
      (l) => DeleteAddressFailure(msg: l.msg),
      (r) {
