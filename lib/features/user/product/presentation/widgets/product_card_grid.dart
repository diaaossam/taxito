    ...
    if (info['isAdd'] == true) {
      if (cartCubit.cartList.isNotEmpty) {
        if (cartCubit.cartList.first.productModel?.supplier?.id !=
          ...
          final response = await ProductHelper().showSupplierWarningDialog(
            context,
            currentSupplierName:
  ...
  Widget build(BuildContext context) {
    return BlocListener<CartCubit, CartState>(
      ...
      child: OpenContainer(
            ...
            children: [
                ...
                children: [
                  ClipRRect(
                    ...
                    child: AppImage.network(
                      height: widget.imageHeight ?? SizeConfig.bodyHeight * .18,
                          ...
                          child: RichText(
                            text: TextSpan(
                              children: [
                                TextSpan(
                                  text:
                                      '${widget.productModel.percentageDiscount}%',
                                  style: TextStyle(
                  ...
                  Positioned.directional(
                    ...
                    child: LikeButtonDesign(
                      key: ValueKey("Product${widget.productModel.id}"),
                      onTapped: widget.onTapped,
                      isLiked: isLiked ?? widget.isLiked,
                    ),
                  ),
                  Positioned.directional(
                    end: 5,
              ...
              Row(
                children: [
                  AppText(
                    text:
                        "${formatPrice(price: widget.productModel.price.toString())} ${context.localizations.iqd}",
                    textSize: 9,
                    fontWeight: FontWeight.w600,
                  ),
                  10.horizontalSpace,
                  if (widget.productModel.oldPrice != null &&
                    ...
                    AppText(
                      text:
                          "${formatPrice(price: widget.productModel.oldPrice.toString())} ${context.localizations.iqd}",
        ...
        openBuilder: (context, action) => ProductDetailsScreen(
          productModel: widget.productModel,
          supplierId: widget.supplierId,
          productId: widget.productModel.id,
          onFavouriteToggled: (data) => setState(() => isLiked = data),
        ),
      ),
    );
  }
