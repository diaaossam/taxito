...
abstract class ProductRemoteDataSource {
  Future<ApiSuccessResponse> toggleWishlist(
      {required num id, required String type});
...
class ProductRemoteDatasourceImpl implements ProductRemoteDataSource {
  ...
  Future<ApiSuccessResponse> toggleWishlist(
    ...
    final response = await dioConsumer
        .put(path: "${EndPoints.favourites}/$id", body: {"type": type});
  ...
  Future<ApiSuccessResponse> getProducts(
    ...
    final response =
        await dioConsumer.post(path: EndPoints.products, body: params.toJson());

    final List<ProductModel> list =
        response['data'].map<ProductModel>((element) {
  ...
  Future<ApiSuccessResponse> submitReview(
    ...
    final response = await dioConsumer.post(
        path: "${EndPoints.rateOrder}/${map['id']}", body: map);
  ...
  Future<ApiSuccessResponse> getFavourite(
    ...
    final List<FavouriteResponse> list =
        response['data'].map<FavouriteResponse>((element) {
      FavouriteResponse favouriteResponse;
      if (type == 1) {
        favouriteResponse = FavouriteResponse(
            type: type, productModel: ProductModel.fromJson(element));
      } else {
        favouriteResponse = FavouriteResponse(
            type: type, supplierModel: SupplierModel.fromJson(element));
      }
