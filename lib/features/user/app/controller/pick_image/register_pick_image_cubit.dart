...
import 'package:aslol/features/app/domain/use_case/delete_image_use_case.dart';
import 'package:aslol/features/app/domain/use_case/upload_image_use_case.dart';
...
class PickImageCubit extends Cubit<PickImageState> {
  ...
  Future<void> uploadImage(
      {required File file, String? faceType, String? path}) async {
    emit(UploadEditImageLoading());
    final response = await uploadImageUseCase(file: file);
    emit(response.fold((l) => UploadEditImageFailure(msg: l.msg),
  ...
  Future<void> deleteImage({required int id}) async {
    ...
    emit(response.fold((l) => DeleteImageFailure(msg: l.msg),
